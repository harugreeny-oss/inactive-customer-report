...
# 비활성 고객사 리스트업 스킬 (v3.1 — CSV 익스포트 + 지난 달 컨택 룰 강화)

## v3.1 변경 사항 (2026-05-19)

- **지난 달 컨택 판단 룰 강화**: 단순 존재 여부 → `이전 달 리스트 존재 + 상태 IN {메일 발송 완료, 회신 완료}` 일 때만 `미해당`.
  - 이전 달 추출만 되고 컨택이 진행되지 않은 (`상태 = 추출`) 회사는 이번 달도 컨택 대상이므로 `해당`으로 분류.
  - `미해당` 의미: "이번 달 컨택 대상에서 제외" (지난 달에 이미 컨택 진행됨).
  - `해당` 의미: "이번 달 컨택 필요" (지난 달 미컨택 or 리스트에 없었음).
...
이번 달 고객사 중 이전 달에서 이미 컨택이 진행된 회사를 가려내기 위해 조회한다.
...
3. 이번 달 비활성 목록(집합 A 전체)과 교차하여 `{회사명: "해당"/"미해당"}` 맵 생성:
   - **`미해당` 조건 (이번 달 컨택 제외)**: 이전 달 리스트에 회사가 있고 + 그 행의 상태가 `메일 발송 완료` 또는 `회신 완료`
   - **`해당` 조건 (이번 달 컨택 필요)** — 이하 모든 경우:
     - 이전 달 리스트에 없는 신규 회사
     - 이전 달 리스트에 있지만 상태가 `추출`인 회사 (지난 달 추출만 됐고 컨택 미진행)
     - 이전 달 페이지 자체가 존재하지 않는 경우
4. `count_prev_no_contact` = 맵에서 `"미해당"`인 회사 수 (= 지난 달 컨택 진행 완료 건수, Notion 요약에 사용)
5. `count_new_contact_target` = 65 - count_prev_no_contact (= 이번 달 신규 컨택 대상 건수)

> ⚠️ 지난 달 컨택 값은 반드시 실제 이전 달 리스트의 `회사명 + 상태` 조회 결과를 기반으로 입력한다. 이전 달 페이지가 없거나 행이 없는 경우 자동으로 `해당` 처리 (공란 금지).
>
> 의미: `해당` = 이번 달 컨택 필요 / `미해당` = 지난 달 이미 컨택 진행됨, 이번 달 제외.
...
| 이번 달 신규 컨택 대상 (지난 달 컨택=해당) | {count_new_contact_target}개사 |
| 지난 달 컨택 진행 완료 (지난 달 컨택=미해당) | {count_prev_no_contact}개사 |

- 조회 기간: 최근 90일 ({start_date} ~ {end_date}, 총합 기준)
- 집합 A 조건: 신규지원자 90일 총합 < 5 (enterprise)
- AND 조건: 집합 A 중 합불처리(합격+불합격) 90일 총합 = 0 → 조건에 `충족` 표시
- 지난 달 컨택 룰: `지난 달 리스트 존재 + 상태 IN {메일 발송 완료, 회신 완료}` → `미해당`, 그 외 → `해당`
